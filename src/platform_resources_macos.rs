...
use objc::{class, sel, sel_impl, msg_send, runtime::{Object, Sel, NO, YES}};
...
impl PlatformResources {
    ...
    pub fn set_clipboard(&self, text: &[u8]) {
        let clipboard: *mut Object = unsafe {
            msg_send![class!(NSPasteboard), generalPasteboard]
        };

        unsafe {
            let string: *mut Object = msg_send![class!(NSString), alloc];
            let allocated_string: *mut Object = msg_send![string, initWithBytes:text.as_ptr() length:text.len() encoding:4];
            let _: () = msg_send![clipboard, clearContents];
            let _: () = msg_send![clipboard, setString:allocated_string forType:NSPasteboardTypeString];
        }
    }
